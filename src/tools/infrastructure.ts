...
export function registerInfrastructureTools(server: McpServer) {
  server.tool(
    ...
    async (params) => {
      const result = await list<Record<string, unknown>>("/api/devices/routers/", {
        params: {
          name: params.name,
  ...
  server.tool(
    ...
    async (params) => {
      const result = await list<Record<string, unknown>>("/api/peering/internet-exchanges/", {
        params: {
          q: params.search,
          status: params.status,
          local_autonomous_system_id: params.local_autonomous_system_id,
  ...
  server.tool(
    ...
    async (params) => {
      const result = await list<Record<string, unknown>>("/api/net/connections/", {
        params: {
          q: params.search,
          status: params.status,
